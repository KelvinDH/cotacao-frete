...
import { FreightMap, TruckType } from "@/api/entities";
...
export default function QuotePage() {
  ...
  const loadTruckTypes = async () => {
    ...
    // Set initial truck type based on default loading mode
    if (types.length > 0) {
  ...
  // Get truck types filtered by current loading mode
  ...
  const handleLoadingModeChange = (mode) => {
    ...
    setQuote(prev => ({
      ...prev,
      loadingMode: mode,
      // Reset truck type and set to first available for new mode if exists
  ...
  const loadCarriers = async () => {
    ...
    const carrierUsers = users.filter(user => user.userType === 'carrier' || user.email === 'anasdinato@gmail.com');
    setCarriers(carrierUsers);
    // By default, select all carriers
    setSelectedCarriers(carrierUsers.map(c => c.carrierName || (c.email === 'anasdinato@gmail.com' ? 'Ana Transportes' : '')));
  };
            ...
            <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
              <div className="flex justify-between items-center mb-4">
                ...
                <div className="flex gap-2">
                  <button
                    type="button"
                    onClick={() => setSelectedCarriers(carriers.map(c => c.carrierName || (c.email === 'anasdinato@gmail.com' ? 'Ana Transportes' : '')))}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                {carriers.map((carrier) => {
                  const carrierName = carrier.carrierName || (carrier.email === 'anasdinato@gmail.com' ? 'Ana Transportes' : '');
                  ...
                  return (
                    ...
                    >
                      ...
                      <Badge className={carrier.carrierType === 'paletizados' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800'}>
                        {carrier.carrierType === 'paletizados' ? 'Paletizados' : 'BAG'}
                      </Badge>
                    </label>
                  );
                })}
